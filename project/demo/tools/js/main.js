 ...
 $(window).resize(function(){
    ...
    c.addListener("click", function(t, evt) {
        ...
        if ($(el).parents('.v3editor').size() > 0) {
            ...
            clickPop.render();
            var html = clickPop.formatHtml('<nobr class="otf-poptools">' + '<span onclick="$$._remove()" stateful>' + '删除该样式</span>' + '<span onclick="$$._blank()" stateful>' + '样式后插入空行</span>' + '<span onclick="$$._preblank()" stateful>' + '样式前插入空行</span>' + '</nobr>');
	...
	var clickPop = new baidu.editor.ui.Popup({content: "",editor: c,_remove: function() {
    	$(clickPop.anchorEl).remove();
    	clickPop.hide();
	},_copy: function() {
