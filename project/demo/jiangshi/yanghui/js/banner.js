...
var banner={
 ...
 move:function(obj,prop,val,type,spd,fn){//运动函数，spd为运动需要的时间，时间越大速度越小
  ...
  obj[prop+'Timer']=setInterval(function(){
   if(t<d){obj.style[prop]=Math[m](banner[st](++t,b,c,d))+'px';}
 ...
 setting:function(par){
  if(window.attachEvent){window.attachEvent('onload',function(){banner[par.style](par)});}
　　  else{window.addEventListener('load',function(){banner[par.style](par)},false);}
 },
 bannerimg:function(par){
  ...
  var run = function(idx) {
   ...
   for(var i=0;i<c;i++){
    ...
    banner.move(banner.$$('p',pic[i])[0],'top',-N*c*h-i*h,'easeOut',spd);
