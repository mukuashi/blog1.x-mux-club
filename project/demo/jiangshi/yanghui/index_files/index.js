...
function quiver(ele,times){
	for(var i = 0; i < times ;i++){
		$(ele).animate({top:'-50px'},100);
		$(ele).animate({top:'-20px'},100);
		$(ele).animate({top:'-40px'},100);
	}	
}
$(function(){
	...
	var bBH = new Array();
	var banner;
	var bannerBgColor = ["#66ccff"]; 
	var bannerImg = ["/style/images/kuajing.png"];//定义左边的图片
	var bannerIcon = ["/style/images/banner_home_right_icon_01.png"];
	for(var i = 0 ; i < 1 ; i++){
	...
	var bannerNumber = $(".bannerInner div").length - 5;
	...
	$(".CButtonRight").click(function(){
		...
		for(var j = 0; j < commentIndex ; j++){
			$(".commentContent").eq(j).animate({marginLeft:"-476px",opacity:0},300);
		}
	...
	function changWallPaper(btnIndex){
			...
			switch(bannerNowIndex){
				case 0:	
					createBanner(0);
					$(".bannerImage").animate({top:"50",opacity:1},chageTime(),chageState());
	...
	$(".CButtonLeft , .CButtonRight , #LeftArr , #RightArr").hover(function(){$(this).css({background:"#79d179"})},function(){$(this).css({background:"#f3f3f3"});})
...
if(browser.versions.mobile || browser.versions.iPhone || browser.versions.android){
	$(".Nav li, .Nav a").css("width","196px");
	$(".Nav,.width ").css("width","980px");
	$(".PartFour li").css("width","190px");
	$(".Comment").css("width","400px");
	$(".footerPart,.footerPart2,.index_pro_wrap2 li").css("width","280px");
	$("body").css({"width":"980px","overflow":"scroll"});
}	
})